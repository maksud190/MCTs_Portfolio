...
const getApprovalBadge = (project) => {
  if (project.approvalStatus === "approved" || project.isApproved === true) {
    return (
      <div className="absolute top-2 left-2 bg-green-500/90 text-white px-3 py-1 rounded-xl text-xs font-bold flex items-center gap-1 shadow-lg z-10">
    ...
    return (
      <div className="absolute top-2 left-2 bg-yellow-500/90 text-white px-3 py-1 rounded-xl text-xs font-bold flex items-center gap-1 shadow-lg z-10">
    ...
    return (
      <div className="absolute top-2 left-2 bg-red-500/90 text-white px-3 py-1 rounded-xl text-xs font-bold flex items-center gap-1 shadow-lg z-10">
        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
          ...
          {projectsLoading ? (
            ...
            <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 xl:columns-4 gap-6">
              {projects.map((p) => (
                <div key={p._id} className="break-inside-avoid mb-6">
                  <div className="relative group">
                    {/* 🔥 ADD APPROVAL BADGE (only on own profile) */}
                    {isOwnProfile && getApprovalBadge(p)}
                    ...
                    {/* 🔥 ADD REJECTION REASON DISPLAY (if rejected) */}
                    {isOwnProfile && p.approvalStatus === "rejected" && p.rejectionReason && (
                      <div className="mt-2 p-3 bg-red-50 border-2 border-red-200 rounded-xl">
                        ...
                        <p className="text-xs text-red-600 font-medium">{p.rejectionReason}</p>
                      </div>
                    )}
                  </div>
                </div>
